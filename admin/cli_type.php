...
{
			// do field validation
			if (document.client_type.client_type.value == "") {
				alert( "Enter the Description" );
				document.client_type.client_type.focus();
				return(false);
			}
			else {
				document.client_type.submit();
				return(true);
			}
}
function ComfirmCancel(){
   ...
   if(r==true){
      ...
      window.location.href = "cli_type.php";
...
//update service required order
        ...
        $sql_update_order =mysql_query("update `client_type` set `order`=".$_POST['order']." where `client_type_id`=".$_POST['svrcode']);
...
<form action="cli_type.php" method="post">
...
<option value="">All Fields</option>
 <option value="<?php echo "client_type" ?>"<?php if ($filterfield == "client_type") { echo "selected"; } ?>>Description</option>
</select></td>
...
<?php
  if($access_file_level['stf_Add']=="Y")
	  {
?>
<a href="cli_type.php?a=add" class="hlight"  ><img src="images/add.gif" alt="Add" name="Add" title="Add" align="absbottom">&nbsp;Add Record</a>
<?php } ?>
<?php showpagenav($page, $pagecount); ?><br>
<br>
<table class="fieldtable" cellspacing="1" cellpadding="5" align="center" width="100%">
<tr class="fieldheader">
 <th align="left" class="fieldheader"><a href="cli_type.php?order=<?php echo "client_type" ?>&type=<?php echo $ordtypestr ?>">Description</a></td>
<th width="25%" class="fieldheader"><a href="cli_type.php?order=<?php echo "order" ?>&type=<?php echo $ordtypestr ?>">Order</a></th>
...
?>
	<form action="cli_type.php<?php echo $updatequery; ?>" method="post">
		<select name="order" onChange="if(confirm('Save?')){this.form.gridedit.click();} else { location.href='cli_type.php?a=reset'}"  ><option value="0">Select Order</option>
		<?php
			//$count = sql_getrecordcount();
		$count = mysql_num_rows($res);
			for($c=1; $c<=$count; $c++)
		...
		<?php } ?>
				...
				<input type="hidden" name="svrcode" value="<?php echo $row["client_type_id"]; ?>">
...
<?php
  if($access_file_level['stf_View']=="Y")
	  {
?>
<td align="center"><a href="cli_type.php?a=view&recid=<?php echo $i ?>"><img src="images/view.png" border="0"  alt="View" name="View" title="View" align="middle" /></a></td>
<?php }?>
<?php
  if($access_file_level['stf_Edit']=="Y")
	  {
?>
<td align="center"><a href="cli_type.php?a=edit&recid=<?php echo $i ?>"><img src="images/edit.png" border="0"  alt="Edit" name="Edit" title="Edit" align="middle" /></a></td>
<?php } ?>
<?php
  if($access_file_level['stf_Delete']=="Y")
	  {
?>
<td align="center"><a onClick="performdelete('cli_type.php?mode=delete&recid=<?php echo htmlspecialchars($row["client_type_id"]) ?>'); return false;" href="#"><img src="images/erase.png" border="0"  alt="Delete" name="Delete" title="Delete" align="middle" /></a></td>
<?php }?>
</tr>
<?php
	$countRow++;
...
<form action="cli_type.php?a=view&recid=<?php echo $recid ?>" method="post">
...
<td class="dr"><?php echo htmlspecialchars($row["client_type"]) ?></td>
</tr>
 <tr>
...
<td class="dr"><input type="text" name="client_type" maxlength="50" value="<?php echo str_replace('"', '&quot;', trim($row["client_type"])) ?>">
<a class="tooltip" href="#"><img src="images/help.png"><span class="help">Name of the entity type.</span></a>
</td>
</tr>
<tr>
<td class="hr">Order</td>
...
<tr>
 <?php if ($page > 1) { ?>
<td><a href="cli_type.php?page=<?php echo $page - 1 ?>"><span style="color:#208EB3; ">&lt;&lt;&nbsp;</span></a>&nbsp;</td>
<?php } ?>
<?php
  global $pagerange;
...
<?php } else { ?>
<td><a href="cli_type.php?page=<?php echo $j ?>"><span class="nav_links"><?php echo $j ?></span></a></td>
<?php } } } else { ?>
<td><a href="cli_type.php?page=<?php echo $startpage ?>"><span class="nav_links"><?php echo $startpage ."..." .$count ?></span></a></td>
<?php } } } ?>
<?php if ($page < $pagecount) { ?>
<td>&nbsp;<a href="cli_type.php?page=<?php echo $page + 1 ?>"><span class="nav_links">&nbsp;&gt;&gt;</span></a>&nbsp;</td>
<?php } ?>
</tr>
</table>
<?php } ?>
...
<tr>
 <?php if ($recid > 0) { ?>
<td><a href="cli_type.php?a=<?php echo $a ?>&recid=<?php echo $recid - 1 ?>"><span style="color:#208EB3; ">&lt;&nbsp;</span></a></td>
...
?>
<div class="frmheading">
	<h1><?php echo $title?> Entity Type</h1>
</div>
...
<form enctype="multipart/form-data" action="cli_type.php" method="post" name="client_type" onSubmit="return validateFormOnSubmit()">
...
if($access_file_level['stf_Add']=="Y")
 {
?>
<td><a href="cli_type.php?a=add" class="hlight">Add Record</a></td>
<?php } ?>
<?php
if($access_file_level['stf_Edit']=="Y")
 {
?>
<td><a href="cli_type.php?a=edit&recid=<?php echo $recid ?>" class="hlight">Edit Record</a></td>
<?php } ?>
<?php
if($access_file_level['stf_Delete']=="Y")
 {
?>
<td><a onClick="performdelete('cli_type.php?mode=delete&recid=<?php echo htmlspecialchars($row["client_type_id"]) ?>'); return false;" href="#"  class="hlight">Delete Record</a></td>
<?php } ?>
</tr>
</table>
<?php
...
<form enctype="multipart/form-data" action="cli_type.php?a=<?php echo $_GET['a']?>&recid=<?php echo $_GET['recid']?>" method="post" name="client_type" onSubmit="return validateFormOnSubmit()">
<input type="hidden" name="sql" value="update">
<input type="hidden" name="xid" value="<?php echo $row["client_type_id"] ?>">
...
function sql_select()
{
  ...
  $sql = "SELECT `client_type_id`, `client_type`, `order` FROM `client_type`";
  if (isset($filterstr) && $filterstr!='' && isset($filterfield) && $filterfield!='') {
    $sql .= " where " .$commonUses->sqlstr($filterfield) ." like '" .$filterstr ."'";
  } elseif (isset($filterstr) && $filterstr!='') {
    $sql .= " where (`client_type_id` like '" .$filterstr ."') or (`client_type` like '" .$filterstr ."')";
  }
...
function sql_insert()
{
  global $_POST;
  global $commonUses;
  $duplicate_entry=$commonUses->checkDuplicate('client_type','client_type',str_replace("'","''",stripslashes(@$_POST["client_type"])));

 if($duplicate_entry==0)
 {
  $sql = "insert into `client_type` (`client_type_id`, `client_type`, `order`) values (" .$commonUses->sqlvalue(@$_POST["client_type_id"], false).", '" .str_replace("'","''",stripslashes(@$_POST["client_type"])) ."', " .$commonUses->sqlvalue(@$_POST["order"], false).")";
...
function sql_update()
{
  ...
  $sql = "update `client_type` set `client_type`='" .str_replace("'","''",stripslashes(@$_POST["client_type"])) ."',`order`=" .$commonUses->sqlvalue(@$_POST["order"], false)." where " .primarykeycondition();
  mysql_query($sql) or die(mysql_error());
}

function sql_delete($client_type_id)
{
   $sql = "delete from `client_type` where client_type_id='".$client_type_id."'";
  if(!mysql_query($sql))
