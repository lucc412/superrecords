...
{
			// do field validation
			if (document.leadstatus.cls_Description.value == "") {
				alert( "Enter the Description" );
				document.leadstatus.cls_Description.focus();
				return(false);
			}
			else {
...
function ComfirmCancel(){
   ...
   if(r==true){
      ...
      window.location.href = "cls_clientleadstatus.php";
...
//update client status order
        ...
        $sql_update_order =mysql_query("update `cls_clientleadstatus` set `cls_Order`=".$_POST['cls_Order']." where cls_Code=".$_POST['clscode']);
...
?>
<br>
<span class="frmheading">Lead Status</span>
<hr size="1" noshade>
<form action="cls_clientleadstatus.php" method="post">
...
<option value="">All Fields</option>
 <option value="<?php echo "cls_Description" ?>"<?php if ($filterfield == "cls_Description") { echo "selected"; } ?>>Description</option>
</select></td>
...
<td><a href="cls_clientleadstatus.php?a=reset" class="hlight">Reset Filter</a></td>
</tr>
</table>
</form>
<p>&nbsp;</p>
...
<tr>
<td>
<a href="cls_clientleadstatus.php?a=add" class="hlight">
<?php 
  if($access_file_level['stf_Add']=="Y")
	  {  
?>
...
<tr class="fieldheader">
 ...
 <th class="fieldheader"><a   href="cls_clientleadstatus.php?order=<?php echo "cls_Description" ?>&type=<?php echo $ordtypestr ?>">Description</a></th>
 <th class="fieldheader"><a href="cls_clientleadstatus.php?order=<?php echo "cls_Order" ?>&type=<?php echo $ordtypestr ?>">Order</a></th>
...
?>
<tr>
 <td class="<?php echo $style ?>"><?php echo htmlspecialchars($row["cls_Description"]) ?></td>
 <td>
<?php
                                if($_GET['page']!="")
...
?>
                                <form action="cls_clientleadstatus.php<?php echo $updatequery; ?>" method="post">
                                    <select name="cls_Order" onChange="if(confirm('Save?')){this.form.gridedit.click();} else { location.href='cls_clientleadstatus.php?a=reset'}"  ><option value="0">Select Order</option>
                                    <?php
                                        //$count = sql_getrecordcount();
                                    $count = mysql_num_rows($res);
                                        for($c=1; $c<=$count; $c++)
                                            {
                                                $val = $c;
                                                if ($row["cls_Order"] == $val) {$selstr = " selected"; } else {$selstr = ""; }

                                    ?>
                                        <option value="<?php echo $c ?>"<?php echo $selstr ?>><?php echo $c ?></option>
                                    <?php } ?>
                                    </select>
                                            <input type="hidden" name="clscode" value="<?php echo $row["cls_Code"]; ?>">
...
<a href="cls_clientleadstatus.php?a=view&recid=<?php echo $i ?>">
...
<?php 
  if($access_file_level['stf_Edit']=="Y")
	  {  
?><td>
<a href="cls_clientleadstatus.php?a=edit&recid=<?php echo $i ?>">
...
<?php 
  if($access_file_level['stf_Delete']=="Y")
	  {  
?><td>
<a onClick="performdelete('cls_clientleadstatus.php?mode=delete&recid=<?php echo htmlspecialchars($row["cls_Code"]) ?>'); return false;" href="#">
...
<td class="dr"><?php echo htmlspecialchars($row["cls_Description"]) ?></td>
</tr>
 <tr>
<td class="hr">Order</td>
<td class="dr"><?php echo htmlspecialchars($row["cls_Order"]) ?></td>
</tr>
...
<td class="dr"><input type="text" name="cls_Description" maxlength="50" value="<?php echo str_replace('"', '&quot;', trim($row["cls_Description"])) ?>">
...
<tr>
 <?php if ($page > 1) { ?>
<td><a href="cls_clientleadstatus.php?page=<?php echo $page - 1 ?>"><span style="color:#208EB3; ">&lt;&lt;&nbsp;</span></a>&nbsp;</td>
<?php } ?>
<?php
  global $pagerange;
...
<?php } else { ?>
<td><a href="cls_clientleadstatus.php?page=<?php echo $j ?>"><span class="nav_links"><?php echo $j ?></span></a></td>
<?php } } } else { ?>
<td><a href="cls_clientleadstatus.php?page=<?php echo $startpage ?>"><span class="nav_links"><?php echo $startpage ."..." .$count ?></span></a></td>
<?php } } } ?>
<?php if ($page < $pagecount) { ?>
<td>&nbsp;<a href="cls_clientleadstatus.php?page=<?php echo $page + 1 ?>"><span class="nav_links">&nbsp;&gt;&gt;</span></a>&nbsp;</td>
<?php } ?>
</tr>
</table>
<?php } ?>
...
<tr>
 <?php if ($recid > 0) { ?>
<td><a href="cls_clientleadstatus.php?a=<?php echo $a ?>&recid=<?php echo $recid - 1 ?>"><span style="color:#208EB3; ">&lt;&nbsp;</span></a></td>
...
<form enctype="multipart/form-data" action="cls_clientleadstatus.php" method="post" name="leadstatus" onSubmit="return validateFormOnSubmit()">
...
<table class="bd" border="0" cellspacing="1" cellpadding="4">
<tr>
<td><a class="hlight"  href="cls_clientleadstatus.php?a=add">Add Record</a></td>
<td><a class="hlight"  href="cls_clientleadstatus.php?a=edit&recid=<?php echo $recid ?>">Edit Record</a></td>
<td><a class="hlight"  onClick="performdelete('cls_clientleadstatus.php?mode=delete&recid=<?php echo htmlspecialchars($row["cls_Code"]) ?>'); return false;" href="#" >Delete Record</a></td>
</tr>
</table>
<?php
  mysql_free_result($res);
...
<br>
<form enctype="multipart/form-data" action="cls_clientleadstatus.php?a=<?php echo $_GET['a']?>&recid=<?php echo $_GET['recid']?>" method="post" name="leadstatus" onSubmit="return validateFormOnSubmit()">
<input type="hidden" name="sql" value="update">
<input type="hidden" name="xcls_Code" value="<?php echo $row["cls_Code"] ?>">
...
function sql_select()
{
  ...
  $sql = "SELECT `cls_Code`, `cls_Description`, `cls_Order` FROM `cls_clientleadstatus`";
  if (isset($filterstr) && $filterstr!='' && isset($filterfield) && $filterfield!='') {
    $sql .= " where " .$commonUses->sqlstr($filterfield) ." like '" .$filterstr ."'";
  } elseif (isset($filterstr) && $filterstr!='') {
    $sql .= " where (`cls_Code` like '" .$filterstr ."') or (`cls_Description` like '" .$filterstr ."')";
  }
...
function sql_insert()
{
  global $_POST;
  global $commonUses;
  $duplicate_entry=$commonUses->checkDuplicate('cls_clientleadstatus','cls_Description',str_replace("'","''",stripslashes(@$_POST["cls_Description"])));
  if($duplicate_entry==0)
   {
  $sql = "insert into `cls_clientleadstatus` (`cls_Code`, `cls_Description`,`cls_Order`) values (" .$commonUses->sqlvalue(@$_POST["cls_Code"], false).", '" .str_replace("'","''",stripslashes(@$_POST["cls_Description"])) ."', " .$commonUses->sqlvalue(@$_POST["cls_Order"], false).")";
...
function sql_update()
{
  global $_POST;
  global $commonUses;
  $sql = "update `cls_clientleadstatus` set `cls_Description`='" .str_replace("'","''",stripslashes(@$_POST["cls_Description"])) ."',`cls_Order`=" .$commonUses->sqlvalue(@$_POST["cls_Order"], false)." where " .primarykeycondition();
  mysql_query($sql) or die(mysql_error());
}

function sql_delete($id)
{
   $sql = "delete from `cls_clientleadstatus` where cls_Code='".$id."'";
  if(!mysql_query($sql))
...
function primarykeycondition()
{
  ...
  $pk .= " = " .$commonUses->sqlvalue(@$_POST["xcls_Code"], false);
  };
  $pk .= ")";
  return $pk;
}
