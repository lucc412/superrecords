...
{
			// do field validation
			if (document.clientstatus.cst_Description.value == "") {
				alert( "Enter the Description" );
				document.clientstatus.cst_Description.focus();
				return(false);
			}
			else {
				document.clientstatus.submit();
				return(true);
			}
}
function ComfirmCancel(){
   ...
   if(r==true){
      ...
      window.location.href = "cst_clientstatus.php";
...
<?php
   //Get FormCode
  $formcode=$commonUses->getFormCode("Client Status");

//update client status order
        if(isset($_POST['gridedit']) && $_POST['gridedit']=="save")
        $sql_update_order =mysql_query("update `cst_clientstatus` set `cst_Order`=".$_POST['cst_Order']." where cst_Code=".$_POST['cstcode']);
...
?>
<br>
<span class="frmheading">Lead Stage</span>
<hr size="1" noshade>
<form action="cst_clientstatus.php" method="post">
...
<option value="">All Fields</option>
 <option value="<?php echo "cst_Description" ?>"<?php if ($filterfield == "cst_Description") { echo "selected"; } ?>>Description</option>
</select></td>
...
<td><a href="cst_clientstatus.php?a=reset" class="hlight">Reset Filter</a></td>
</tr>
</table>
</form>
<p>&nbsp;</p>
...
<tr>
<td>
<a href="cst_clientstatus.php?a=add" class="hlight">
<?php 
  if($access_file_level['stf_Add']=="Y")
	  {  
?>
...
<tr class="fieldheader">
 ...
 <th class="fieldheader"><a   href="cst_clientstatus.php?order=<?php echo "cst_Description" ?>&type=<?php echo $ordtypestr ?>">Description</a></th>
 <th class="fieldheader"><a href="cst_clientstatus.php?order=<?php echo "cst_Order" ?>&type=<?php echo $ordtypestr ?>">Order</a></th>
...
?>
<tr>
 <td class="<?php echo $style ?>"><?php echo htmlspecialchars($row["cst_Description"]) ?></td>
 <td>
<?php
                                if($_GET['page']!="")
...
?>
                                <form action="cst_clientstatus.php<?php echo $updatequery; ?>" method="post">
                                    <select name="cst_Order" onChange="if(confirm('Save?')){this.form.gridedit.click();} else { location.href='cst_clientstatus.php?a=reset'}"  ><option value="0">Select Order</option>
                                    <?php
                                        //$count = sql_getrecordcount();
                                    $count = mysql_num_rows($res);
                                        for($c=1; $c<=$count; $c++)
                                            {
                                                $val = $c;
                                                if ($row["cst_Order"] == $val) {$selstr = " selected"; } else {$selstr = ""; }

                                    ?>
                                        <option value="<?php echo $c ?>"<?php echo $selstr ?>><?php echo $c ?></option>
                                    <?php } ?>
                                    </select>
                                            <input type="hidden" name="cstcode" value="<?php echo $row["cst_Code"]; ?>">
...
<a href="cst_clientstatus.php?a=view&recid=<?php echo $i ?>">
...
<?php 
  if($access_file_level['stf_Edit']=="Y")
	  {  
?><td>
<a href="cst_clientstatus.php?a=edit&recid=<?php echo $i ?>">
...
<?php 
  if($access_file_level['stf_Delete']=="Y")
	  {  
?><td>
<a onClick="performdelete('cst_clientstatus.php?mode=delete&recid=<?php echo htmlspecialchars($row["cst_Code"]) ?>'); return false;" href="#">
...
<td class="dr"><?php echo htmlspecialchars($row["cst_Description"]) ?></td>
</tr>
 <tr>
<td class="hr">Order</td>
<td class="dr"><?php echo htmlspecialchars($row["cst_Order"]) ?></td>
</tr>
...
<td class="dr"><input type="text" name="cst_Description" maxlength="50" value="<?php echo str_replace('"', '&quot;', trim($row["cst_Description"])) ?>">
<a class="tooltip" href="#"><img src="images/help.png"><span class="help">Name of the lead stage.</span></a>
</td>
</tr>
<tr>
<td class="hr">Order</td>
		<td>
			<select name="cst_Order">
...
<tr>
 <?php if ($page > 1) { ?>
<td><a href="cst_clientstatus.php?page=<?php echo $page - 1 ?>"><span style="color:#208EB3; ">&lt;&lt;&nbsp;</span></a>&nbsp;</td>
<?php } ?>
<?php
  global $pagerange;
...
<?php } else { ?>
<td><a href="cst_clientstatus.php?page=<?php echo $j ?>"><span class="nav_links"><?php echo $j ?></span></a></td>
<?php } } } else { ?>
<td><a href="cst_clientstatus.php?page=<?php echo $startpage ?>"><span class="nav_links"><?php echo $startpage ."..." .$count ?></span></a></td>
<?php } } } ?>
<?php if ($page < $pagecount) { ?>
<td>&nbsp;<a href="cst_clientstatus.php?page=<?php echo $page + 1 ?>"><span class="nav_links">&nbsp;&gt;&gt;</span></a>&nbsp;</td>
<?php } ?>
</tr>
</table>
<?php } ?>
...
<tr>
 <?php if ($recid > 0) { ?>
<td><a href="cst_clientstatus.php?a=<?php echo $a ?>&recid=<?php echo $recid - 1 ?>"><span style="color:#208EB3; ">&lt;&nbsp;</span></a></td>
...
<form enctype="multipart/form-data" action="cst_clientstatus.php" method="post" name="clientstatus" onSubmit="return validateFormOnSubmit()">
...
<table class="bd" border="0" cellspacing="1" cellpadding="4">
<tr>
<td><a class="hlight"  href="cst_clientstatus.php?a=add">Add Record</a></td>
<td><a class="hlight"  href="cst_clientstatus.php?a=edit&recid=<?php echo $recid ?>">Edit Record</a></td>
<td><a class="hlight"  onClick="performdelete('cst_clientstatus.php?mode=delete&recid=<?php echo htmlspecialchars($row["cst_Code"]) ?>'); return false;" href="#" >Delete Record</a></td>
</tr>
</table>
<?php
  mysql_free_result($res);
...
<br>
<form enctype="multipart/form-data" action="cst_clientstatus.php?a=<?php echo $_GET['a']?>&recid=<?php echo $_GET['recid']?>" method="post" name="clientstatus" onSubmit="return validateFormOnSubmit()">
<input type="hidden" name="sql" value="update">
<input type="hidden" name="xcst_Code" value="<?php echo $row["cst_Code"] ?>">
...
function sql_select()
{
  ...
  $sql = "SELECT `cst_Code`, `cst_Description`, `cst_Order` FROM `cst_clientstatus`";
  if (isset($filterstr) && $filterstr!='' && isset($filterfield) && $filterfield!='') {
    $sql .= " where " .$commonUses->sqlstr($filterfield) ." like '" .$filterstr ."'";
  } elseif (isset($filterstr) && $filterstr!='') {
    $sql .= " where (`cst_Code` like '" .$filterstr ."') or (`cst_Description` like '" .$filterstr ."')";
  }
...
function sql_insert()
{
  global $_POST;
  global $commonUses;
  $duplicate_entry=$commonUses->checkDuplicate('cst_clientstatus','cst_Description',str_replace("'","''",stripslashes(@$_POST["cst_Description"])));
  if($duplicate_entry==0)
   {
  $sql = "insert into `cst_clientstatus` (`cst_Code`, `cst_Description`,`cst_Order`) values (" .$commonUses->sqlvalue(@$_POST["cst_Code"], false).", '" .str_replace("'","''",stripslashes(@$_POST["cst_Description"])) ."', " .$commonUses->sqlvalue(@$_POST["cst_Order"], false).")";
...
function sql_update()
{
  global $_POST;
  global $commonUses;
  $sql = "update `cst_clientstatus` set `cst_Description`='" .str_replace("'","''",stripslashes(@$_POST["cst_Description"])) ."',`cst_Order`=" .$commonUses->sqlvalue(@$_POST["cst_Order"], false)." where " .primarykeycondition();
  mysql_query($sql) or die(mysql_error());
}

function sql_delete($id)
{
   $sql = "delete from `cst_clientstatus` where cst_Code='".$id."'";
  if(!mysql_query($sql))
...
function primarykeycondition()
{
  ...
  $pk .= " = " .$commonUses->sqlvalue(@$_POST["xcst_Code"], false);
  };
  $pk .= ")";
  return $pk;
}
