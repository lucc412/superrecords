                            ...
                            function sql_select(&$count=0)
                            {
                                  ...
                                  global $CrossDbcontent;
                                      ...
                                      if (!$wholeonly && isset($wholeonly) && $filterstr!='') $filterstr = "%" .$filterstr ."%";
                                            ...
                                            $sql = "SELECT * FROM (SELECT t1.*, cls1.`cls_Description` AS `cli_lead_status`, lp14.`cst_Description` AS `lp_cli_Stage`, m1.`moc_Description` AS `lp_cli_MOC`, CONCAT(c1.`con_Firstname`,' ',c1.`con_Lastname`) AS `lp_cli_Salesperson`, j1.`client_name` AS `cso_client_name`, CONCAT(cn1.con_Firstname,' ',cn1.con_Lastname) AS `cso_contact_name`, CONCAT(c2.`con_Firstname`,' ',c2.`con_Lastname`) AS `cso_lead_person`, src.`src_Description` AS `source_name` FROM `cso_cross_sales_opportunity` AS t1 LEFT OUTER JOIN `stf_staff` AS s2 ON (t1.`cso_sales_person` = s2.`stf_Code`) LEFT OUTER JOIN `con_contact` AS c1 ON (s2.`stf_CCode` = c1.`con_Code`) LEFT OUTER JOIN `moc_methodofcontact` AS m1 ON (t1.`cso_method_of_contact` = m1.`moc_Code`) LEFT OUTER JOIN `cls_clientleadstatus` AS cls1 ON (t1.`cso_lead_status` = cls1.`cls_Code`) LEFT OUTER JOIN `cst_clientstatus` AS lp14 ON (t1.`cso_stage` = lp14.`cst_Code`) LEFT OUTER JOIN client as j1 ON (t1.cso_client_code = j1.client_id) LEFT OUTER JOIN con_contact as cn1 ON (t1.cso_contact_code = cn1.con_Code) LEFT OUTER JOIN `stf_staff` AS s3 ON (t1.`cso_generated_lead` = s3.`stf_Code`) LEFT OUTER JOIN `con_contact` AS c2 ON (s3.`stf_CCode` = c2.`con_Code`) LEFT OUTER JOIN `src_source` AS src ON (t1.`cso_source` = src.`src_Code`) ".$condition." ) subq";
