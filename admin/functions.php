...
function get_all_dates ( $date, $rpt_type, $interval = 1, $Byxxx = '',
  $Count = 99, $Until = null, $Wkst = 'MO', $ex_days = '', $inc_days = '',
  $jump = '', $induedate ) {
  ...
  if ( $Until == null && $Count == 99 ) {
    ...
    $realend = mktime ( $hour, $minute, 0, $thismonth, $thisday, $thisyear );
     ...
     $realend = ( $Count != 99
      ? mktime ( 0, 0, 0, 1, 1, 2012 ) // Set $until so some ridiculous value.
      : $Until );
	//print_r("<br>".( floor ( $date / 86400 ) * 86400 ));
