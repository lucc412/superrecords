function validation()
  {
        ...
        frm.action = "sales_opportunity_report.php?aid="+aid;
        frm.submit();
        return false;
  }
function saveCondition(scode,title)
{
     ...
     $(document).ready(function() {
             ...
             $.ajax({
                url: "sales_opportunity_report_ajax.php",
                type:"POST",
                async:true,
                ...
                success: function(msg){
                     ...
                     for(var j=1; j<(msg_field.length); j++) {
                        ...
                        if(msg_field[j]=="cso_client_code") {
                     ...
                     document.getElementById('cso_Title').value = sub;
...
function in_array(p_val,arr)
  ...
  function cond_fields(rep_val)
  {
      ...
      */
      ...
      $(document).ready(function() {
            $.ajax({
                url: "ajax_sales_opportunity.php",
                type:"POST",
                cache: false,
...
{
  document.form1.action = "cso_sales_opportunity_report.php?aid="+aid+"&page="+page;
...
function saveSubmit()
{
    ...
    var sub = document.getElementById('cso_Title').value;
    if(sub=="") {
        alert('Enter Report Name');
        document.getElementById('cso_Title').focus();
        return false;
    }
...
function leadstatusReason(mypage,myname,w,h,scroll,status,old,cid,page,aid) {
     if(status==3) {
     if(document.getElementById('notsuccesrow')) document.getElementById('notsuccesrow').style.display='block';
...
win = window.open(mypage,myname,settings);
     ...
     else if(status!=3 && old=='Not Successful'){
         if(confirm('You are about to re-open the lead. Do you want to continue?')){
         customSave(cid,page,aid,'open',status)
         }
     }
}
function leadclosureWindowVal() {
    if(document.getElementById('notsuccessful_reason').value=="") {
        alert('Enter Reason');
        document.getElementById('notsuccessful_reason').focus();
        return false;
    }
    else return true
}
function getLeadclosure(val)
{
    if(val) document.getElementById('notsuccessful_reason').value = val;
    else document.getElementById('notsuccessful_reason').value = '';
}
