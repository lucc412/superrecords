...
function ComfirmCancel(){
   ...
   if(r==true){
      ...
      window.location.href = "lead_industry.php";
...
<?php
  //Get FormCode
  $formcode=$commonUses->getFormCode("Lead Industry");

  //update Lead Industry order
  if(isset($_POST['gridedit']) && $_POST['gridedit']=="save")
	  $sql_update_order =mysql_query("update `lead_industry` set `order`=".$_POST['order']." where id=".$_POST['svrcode']);
...
<form action="lead_industry.php" method="post">
...
<td><a href="lead_industry.php?a=reset" class="hlight">Reset Filter</a></td>
</tr>
</table>
</form>
 <p>&nbsp;</p>
...
<?php
  if($access_file_level['stf_Add']=="Y")
	  {
?>
<a href="lead_industry.php?a=add" class="hlight"  ><img src="images/add.gif" alt="Add" name="Add" title="Add" align="absbottom">&nbsp;Add Record</a>
<?php } ?>
<?php showpagenav($page, $pagecount); ?><br>
<br>
<table class="fieldtable" cellspacing="1" cellpadding="5" align="center" width="100%">
<tr class="fieldheader">
<th align="left" class="fieldheader"><a href="lead_industry.php?order=<?php echo "description" ?>&type=<?php echo $ordtypestr ?>">Description</a></td>
<th class="fieldheader" width="25%"><a href="lead_industry.php?order=<?php echo "order" ?>&type=<?php echo $ordtypestr ?>">Order</a></th>
...
?>
	<form action="lead_industry.php<?php echo $updatequery; ?>" method="post">
		<select name="order" onChange="if(confirm('Save?')){this.form.gridedit.click();} else { location.href='lead_industry.php?a=reset'}"  ><option value="0">Select Order</option>
		<?php
			//$count = sql_getrecordcount();
		$count = mysql_num_rows($res);
			for($c=1; $c<=$count; $c++)
...
<?php
  if($access_file_level['stf_View']=="Y")
	  {
?>
<td align="center"><a href="lead_industry.php?a=view&recid=<?php echo $i ?>"><img src="images/view.png" border="0"  alt="View" name="View" title="View" align="middle" /></a></td>
<?php }?>
<?php
  if($access_file_level['stf_Edit']=="Y")
	  {
?>
<td align="center"><a href="lead_industry.php?a=edit&recid=<?php echo $i ?>"><img src="images/edit.png" border="0"  alt="Edit" name="Edit" title="Edit" align="middle" /></a></td>
<?php } ?>
<?php
  if($access_file_level['stf_Delete']=="Y")
	  {
?>
<td align="center"><a onClick="performdelete('lead_industry.php?mode=delete&recid=<?php echo htmlspecialchars($row["id"]) ?>'); return false;" href="#"><img src="images/erase.png" border="0"  alt="Delete" name="Delete" title="Delete" align="middle" /></a></td>
<?php }?>
</tr>
<?php
	$countRow++;
...
<form action="lead_industry.php?a=view&recid=<?php echo $recid ?>" method="post">
...
<a class="tooltip" href="#"><img src="images/help.png"><span class="help">Name of the Lead Industry.</span></a>
</td>
</tr>
<tr>
<td class="hr">Order</td>
...
<tr>
 <?php if ($page > 1) { ?>
<td><a href="lead_industry.php?page=<?php echo $page - 1 ?>"><span style="color:#208EB3; ">&lt;&lt;&nbsp;</span></a>&nbsp;</td>
<?php } ?>
<?php
  global $pagerange;
...
<?php } else { ?>
<td><a href="lead_industry.php?page=<?php echo $j ?>"><span class="nav_links"><?php echo $j ?></span></a></td>
<?php } } } else { ?>
<td><a href="lead_industry.php?page=<?php echo $startpage ?>"><span class="nav_links"><?php echo $startpage ."..." .$count ?></span></a></td>
<?php } } } ?>
<?php if ($page < $pagecount) { ?>
<td>&nbsp;<a href="lead_industry.php?page=<?php echo $page + 1 ?>"><span class="nav_links">&nbsp;&gt;&gt;</span></a>&nbsp;</td>
<?php } ?>
</tr>
</table>
<?php } ?>
...
<tr>
 <?php if ($recid > 0) { ?>
<td><a href="lead_industry.php?a=<?php echo $a ?>&recid=<?php echo $recid - 1 ?>"><span style="color:#208EB3; ">&lt;&nbsp;</span></a></td>
...
<form enctype="multipart/form-data" action="lead_industry.php" method="post" name="cli_servicerequired" onSubmit="return validateFormOnSubmit()">
...
if($access_file_level['stf_Add']=="Y")
 {
?>
<td><a href="lead_industry.php?a=add" class="hlight">Add Record</a></td>
<?php } ?>
<?php
if($access_file_level['stf_Edit']=="Y")
 {
?>
<td><a href="lead_industry.php?a=edit&recid=<?php echo $recid ?>" class="hlight">Edit Record</a></td>
<?php } ?>
<?php
if($access_file_level['stf_Delete']=="Y")
 {
?>
<td><a onClick="performdelete('lead_industry.php?mode=delete&recid=<?php echo htmlspecialchars($row["id"]) ?>'); return false;" href="#"  class="hlight">Delete Record</a></td>
<?php } ?>
</tr>
</table>
<?php
...
<form enctype="multipart/form-data" action="lead_industry.php?a=<?php echo $_GET['a']?>&recid=<?php echo $_GET['recid']?>" method="post" name="cli_servicerequired" onSubmit="return validateFormOnSubmit()">
...
function sql_select()
{
  ...
  $sql = "SELECT `id`, `description`, `order` FROM `lead_industry`";
...
function sql_insert()
{
  global $_POST;
  global $commonUses;
  $duplicate_entry=$commonUses->checkDuplicate('lead_industry','description',str_replace("'","''",stripslashes(@$_POST["description"])));

 if($duplicate_entry==0)
 {
  $sql = "insert into `lead_industry` (`id`, `description`, `order`) values (" .$commonUses->sqlvalue(@$_POST["id"], false).", '" .str_replace("'","''",stripslashes(@$_POST["description"])) ."', " .$commonUses->sqlvalue(@$_POST["order"], false).")";
...
function sql_update()
{
  ...
  $sql = "update `lead_industry` set `description`='" .str_replace("'","''",stripslashes(@$_POST["description"])) ."',`order`=" .$commonUses->sqlvalue(@$_POST["order"], false)." where " .primarykeycondition();
  mysql_query($sql) or die(mysql_error());
  header("location:lead_industry.php");
}

function sql_delete($id)
{
   $sql = "delete from `lead_industry` where id='".$id."'";
  if(!mysql_query($sql))
