...
<form action="audit_checklist.php?sql=update" method="POST" name="managechecklist" onSubmit="return validateFormOnSubmit()">

	<table class="tbl" border="0" cellspacing="10" width="70%">
		<tr>
			<td class="hr">Checklist Name <font style="color:red;" size="2">*</font></td>
			<td class="dr">
				<input type="text" name="txtCheckListName" id="txtCheckListName" size="26" value="<?=$arrData['name'];?>">
		...
		<?php if(!isset($arrData['sid'])){?>
			<input type="hidden" id="selParent" name="selParent" value="0">
		<?php } else {?>
		<tr>
		   ...
		   <td>
		   		<select id="selParent" name="selParent">
						...
						while($arrInfo = mysql_fetch_assoc($arrChecklist)) {
							?><option <?=($arrData['cid']==$arrInfo['cid'])?"selected":"";?> value="<?=$arrInfo['cid'];?>"><?=$arrInfo['name'];?></option><? 
						}
					?></select>
		...
		<tr>
		   ...
		   <td>
		   		<select id="order" name="order">
					...
					for($i=1;$i<=$order;$i++){
						?><option value="<?=$i;?>" <?=(isset($arrData['corder']) && $arrData['corder'] == $i)?"selected":"";?>><?=$i;?></option><? 
					} 
				?></select>
				<a class="tooltip" href="#"><img src="images/help.png"><span class="help">While Add SubChecklist you have to select Checklist.</span></a>
			</td>
