...
?><form id="frmTrust" method="post" action="trust_asset.php">
    ...
    <input type="hidden" name="assetCnt" id="assetCnt" value="<?=count($arrHoldTrust)?>">
    <button type="button" id="btnAdd" style="width:105px">Add Asset</button><?
        ...
        foreach ($arrHoldTrust AS $assetId => $assetData) {
            ...
            <table class="fieldtable" width="60%" cellpadding="10px;">
                <tr>
                    ...
                    <td><input type="text" name="txtYear<?=$assetCnt?>" id="txtYear<?=$assetCnt?>" value="<?=$assetData['financial_year']?>" /></td>
                </tr>
                <tr>    
                    <td>Asset</td>
