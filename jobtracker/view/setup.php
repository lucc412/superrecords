        ...
        <?php foreach($subforms['subforms'] as $frm => $frmVal){    
            ?><li>
                ...
                if(!empty($frmVal['subform_url'])) {
                    ?><button style="width: 125px;margin: 0" onclick='window.location.href="setup/<?=$frmVal['subform_url']?>?frmId=<?=$frmVal['subform_id']?>"' >Order Now</button><?
                }
                else {
                    ?><button style="width: 125px;margin: 0" class="orderBtn">Coming Soon</button><?
                }
            ?></li>
        <?php } ?>
        </ul>        
